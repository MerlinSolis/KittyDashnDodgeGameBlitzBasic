...
Global girl_voice_ready
Global snack_ammo_reload_sound
Global cat_purr_sound_loop
Global girl_voice_aah
Dim girl_hurt_sound_list(2)
Dim jump_sound_list(2)
Dim snack_throw_sound_list(2)
Dim heart_zapped_sound_list(2)
Dim health_pickup_sound_list(2)
Dim cat_spawn_sound_list(2)
Dim girl_laugh_sound_list(2)
Load_Sound_Files()

bgm_channel = PlayMusic(ASSET_PATH$+"Sound_FX/Summer gift.mp3")
ChannelVolume bgm_channel,0.25
...
;###############ParticleEffectDataAndMethodsStart#############################################################################

Dim heart_image_list(2)
Load_Rescaled_Hearts()
Type Heart_Particles
	...
	Field life_time
End Type
Function Init_Heart_Particle.Heart_Particles(particle_image,init_x#,init_y#,spawn_time,life_time,angle#,speed#)
	heart_particle.Heart_Particles = New Heart_Particles
	heart_particle\image = particle_image
	heart_particle\x# = init_x#
	heart_particle\y# = init_y#
	heart_particle\spawn_time = spawn_time
	heart_particle\life_time = life_time
	heart_particle\angle# = angle#
	heart_particle\speed# = speed#
	Return heart_particle.Heart_Particles
End Function
Function Update_Heart_Particle(heart_particle.Heart_Particles)
	heart_particle\x# = heart_particle\x# + (Cos(heart_particle\angle#)*heart_particle\speed)
	heart_particle\y# = heart_particle\y# + (Sin(heart_particle\angle#)*heart_particle\speed)
	
	If MilliSecs() > (heart_particle\spawn_time + heart_particle\life_time) Then
		Delete heart_particle
	EndIf
	
End Function
Function Draw_Heart_Particle(heart_particle.Heart_Particles)
	If heart_particle <> Null Then
		DrawImage heart_particle\image,heart_particle\x,heart_particle\y
	EndIf
End Function

Function Spawn_Random_Hearts(x#,y#)
	For heart_num = 0 To Rand(1,100)
		heart_particle.Heart_Particles = Init_Heart_Particle(heart_image_list(Rand(0,2)),x#+(Rand(-100,100)),y#+(Rand(-100,100)),MilliSecs(),Rand(500,1500),Rnd(0,360),Rnd(4,8))
	Next
End Function

Function Load_Rescaled_Hearts()
	original_heart_img = LoadImage(ASSET_PATH+"Gui_Elements/health_heart_01.png")
	MaskImage original_heart_img,0,0,0
	For img_num = 0 To 2
		temp_rescaled_img = CopyImage(original_heart_img)
		scale# = Rnd(0.1,1.0)
		ScaleImage temp_rescaled_img,scale#,scale#
		MidHandle temp_rescaled_img
		heart_image_list(img_num) = temp_rescaled_img
	Next
End Function
...
;##############ParticleEffectDataAndMethodsEnd############################################################################
...
Function Player_Girl_Take_Input(player_instance.Player_Girl)
	...
	ElseIf (KeyHit(K_UP) Or KeyHit(K_W)) And (Not player_instance\bool_jump) Then
		PlaySound jump_sound_list(Rand(0,2))
	...
	If KeyHit(K_SPACE) And player_instance\snack_bullet_count > 0 Then
		PlaySound snack_throw_sound_list(Rand(0,2))
...
Function Init_Obstacle_Kitty_Self.Obstacle_Kitty(kitty_type$)
	...
	If Rand(1,4) = 1 Then
		PlaySound(cat_spawn_sound_list(Rand(0,2)))
	EndIf
End Function
...
Function Obstacle_Kitty_Update(obstacle_kitty.Obstacle_Kitty,player_girl.Player_Girl)
	...
	If obstacle_kitty\x <= -obstacle_kitty\image_width Then
		...
		If player_girl\snack_bullet_count = 0 Then
			PlaySound(snack_ammo_reload_sound)
			player_girl\snack_bullet_count = 3
		EndIf
		kitty_object_count = kitty_object_count - 1
	EndIf
...
While run
	...
	Select game_state
		...
		Case 1
			...
			For obstacle_kitty.Obstacle_Kitty = Each Obstacle_Kitty
				...
				If obstacle_kitty <> Null Then
					If ImagesCollide (player_girl\image,player_girl\x,player_girl\y,0,obstacle_kitty\image,obstacle_kitty\x,obstacle_kitty\y,0) And (player_girl\bool_hit_flag = False) Then
						PlaySound girl_hurt_sound_list(Rand(0,2))
				...
				If snack_bullet_object <> Null Then
					For obstacle_kitty.Obstacle_Kitty = Each Obstacle_Kitty
						If ImagesCollide(snack_bullet_object\image,snack_bullet_object\x,snack_bullet_object\y,0,obstacle_kitty\image,obstacle_kitty\x,obstacle_kitty\y,0) Then
							PlaySound(heart_zapped_sound_list(Rand(0,2)))
							Delete snack_bullet_object
							...
							EndIf
							Spawn_Random_Hearts(obstacle_kitty\x+ImageWidth(obstacle_kitty\image)/2,obstacle_kitty\y+ImageHeight(obstacle_kitty\image)/2)
							Delete obstacle_kitty
							score = score + 500
			...
			For health_heart_object.Health_Heart_Pickup = Each Health_Heart_Pickup
				...
				If health_heart_object <> Null Then
					If ImagesOverlap(health_heart_object\image,health_heart_object\x,health_heart_object\y,player_girl\image,player_girl\x,player_girl\y) Then
						PlaySound(health_pickup_sound_list(Rand(0,2)))
						If player_girl\health_points < 3 Then
			...
			For heart_particle.Heart_Particles = Each Heart_Particles
				Update_Heart_Particle(heart_particle)
				Draw_Heart_Particle(heart_particle)
			Next
		...
		Case 2
			Win_Screen(win_screen_image,normal_font,player_girl)
	End Select
...
Function Win_Screen(win_screen_image,normal_font,player_girl.Player_Girl)
	If score > hi_score Then
		file_out = WriteFile(save_score_dir$+"hi_score.txt")
		hi_score = score
		WriteInt file_out,hi_score
		CloseFile file_out
	EndIf
	purr_channel = PlaySound(cat_purr_sound_loop)
	PlaySound(girl_laugh_sound_list(Rand(0,2)))
	FlushKeys
	Color 10,10,10
	Rect 0,0,SCREEN_WIDTH,SCREEN_HEIGHT,True
...
;		game_state = 2
;	EndIf
	current_level_number = Floor(score/10000)
	next_level_number = current_level_number + 1
	If (current_level_number > old_level_number) And (current_level_number < next_level_number) Then
		win_count = win_count + 1
...
Function Load_Sound_Files()
	snack_ammo_reload_sound = LoadSound(ASSET_PATH$+"Sound_FX/beretta_m9_pistol_reload.wav")
	cat_purr_sound_loop = LoadSound(ASSET_PATH$+"Sound_FX/cat_purr_loop.wav")
	girl_voice_ready = LoadSound(ASSET_PATH$+"Sound_FX/ready_girl_voice.wav")
	girl_voice_aah = LoadSound(ASSET_PATH$+"Sound_FX/girl_aah_sound.wav")
	LoopSound cat_purr_sound_loop
	For sound_num = 0 To 2
		girl_hurt_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/girl_hurt_"+Str(sound_num+1)+".wav")
		SoundVolume girl_hurt_sound_list(sound_num),0.5
		jump_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/jump_"+Str(sound_num+1)+".wav")
		snack_throw_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/snack_throw_"+Str(sound_num+1)+".wav")
		heart_zapped_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/heart_zapped_"+Str(sound_num+1)+".wav")
		health_pickup_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/health_pick_"+Str(sound_num+1)+".wav")
		cat_spawn_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/cat_mew_"+Str(sound_num+1)+".wav")
		SoundVolume cat_spawn_sound_list(sound_num),0.5
		girl_laugh_sound_list(sound_num) = LoadSound(ASSET_PATH$+"Sound_FX/girl_laugh_"+Str(sound_num+1)+".wav")
	Next
End Function
